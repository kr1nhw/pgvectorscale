...
## 3b. Config parity note

The index definitions and sweep parameters are **identical** on both stacks
(verified live against pg_indexes): ivfrq
`WITH (lists=1000, num_bits=1)` (346 MB both), hnsw
`WITH (m=16, ef_construction=64)` (7.9 GB both); sweeps use the same probe /
ef_search values, the same 100 queries and ground truth. Differences are
environmental only (shared_buffers 8GB vs 2GB+LFC, local disk vs pageserver,
3/3 WAL quorum, PG 17.11 vs fork 17.5, hnsw build memory 16GB vs 12GB).

The small recall deltas between stacks (<1-4pp at low probe/ef_search,
converging at high values) come from build randomness — each stack built its
own index instance (random k-means init / graph construction) — not from
configuration drift.
