...
# hnswsq A/B variant of run_sweep.sh: engines are `hnsw` (pgvector) and
# `hnswsq` (this extension); identical protocol so results are comparable.
#
# Usage: run_sweep_hnswsq.sh <psql_bin> <engine:hnsw|hnswsq> <label> <out_csv>
#   psql_bin  psql executable; connection via libpq env vars (PGHOST/PGPORT/...)
#   engine    hnsw   -> sweep hnsw.ef_search over 10..640
#             hnswsq -> sweep hnswsq.ef_search over 10..640
#   label     tag written into the CSV (e.g. hnswsq-plain)
...
# and exactly one usable index: items_10m_hnsw or items_10m_hnswsq.
#
