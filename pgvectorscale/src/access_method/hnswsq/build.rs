//! hnswsq build — the Rust translation of pgvector's `hnswbuild.c`.
//!
...
//! * levels are pinned per row `(seed, tid)` when `hnswsq.build_seed` is
...
pub unsafe fn init_graph(graph: *mut Graph, base: *mut u8, memory_total: usize, tranche: i32) {
    crate::access_method::hnswsq::ptr::store(base, &mut (*graph).head, std::ptr::null_mut::<u8>());
...
unsafe fn add_element_in_memory(base: *mut u8, graph: *mut Graph, element: *mut Element) {
    ...
    crate::access_method::hnswsq::ptr::store(base, &mut (*graph).head, element);
...
unsafe fn update_graph_in_memory(
    ...
    if entry_point.is_none() || (*element).level > (*entry_point.unwrap()).level {
        crate::access_method::hnswsq::ptr::store(base, &mut (*graph).entry_point, element);
    }
}
...
unsafe fn insert_tuple(
    ...
    if (*graph).memory_used + memory_margin >= (*graph).memory_total {
        ...
        if !(*graph).flushed {
            ...
            pgrx::notice!(
                "hnswsq graph no longer fits into maintenance_work_mem after {} tuples",
                indtuples as i64
            );
            flush_pages(build);
        }
    ...
    crate::access_method::hnswsq::ptr::store(base, &mut (*element).value, value_ptr);
...
unsafe fn create_graph_pages(build: &mut BuildState) {
    ...
    while !crate::access_method::hnswsq::ptr::is_null(base, iter) {
        let element = crate::access_method::hnswsq::ptr::access::<Element>(base, iter);
        ...
        if etup_size > pg_sys::BLCKSZ as usize {
            error!("hnswsq: index tuple too large");
        }
    ...
    let entry_point =
        crate::access_method::hnswsq::ptr::access::<Element>(base, (*graph).entry_point);
...
unsafe fn write_neighbor_tuples(build: &BuildState) {
    ...
    while !crate::access_method::hnswsq::ptr::is_null(base, iter) {
        ...
        if !pg_sys::PageIndexTupleOverwrite(
            ...
            error!("hnswsq: failed to overwrite neighbor tuple");
        }
...
unsafe fn begin_parallel(build: &mut BuildState, isconcurrent: bool, request: i32) {
    ...
    pgrx::log!(
        "hnswsq using {} parallel workers for index build",
...
pub unsafe fn init_build_state(
    ...
    if dimensions > MAX_DIM {
        error!(
            "hnswsq: column cannot have more than {} dimensions",
            MAX_DIM
        );
    }
    if ef_construction / 2 < m {
        error!("hnswsq: ef_construction must be greater than or equal to 2 * m");
    }

    let dist_type = resolve_distance_type(index);

    let graph_ctx = PgMemoryContexts::new("hnswsq build graph context");
    let tmp_ctx = PgMemoryContexts::new("hnswsq build temporary context");
    let tranche = register_tranche(c"hnswsq build");
