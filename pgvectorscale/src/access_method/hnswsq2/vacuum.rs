//! hnswsq2 vacuum — the Rust translation of pgvector's `hnswvacuum.c`.
//!
...
use crate::access_method::hnswsq2::insert::update_neighbors_on_disk;
        ...
        while offno <= maxoffno as pg_sys::OffsetNumber {
            ...
            for i in 0..(*ntup).count as usize {
                ...
                if deleting_element(&vac.deleting, *indextid) {
                    ...
                    error!("hnswsq2 graph not repaired");
                }
            }
...
pub unsafe extern "C-unwind" fn ambulkdelete(
    ...
    let mut vac = VacuumState {
        ...
        tmp_ctx: PgMemoryContexts::new("hnswsq2 vacuum temporary context"),
