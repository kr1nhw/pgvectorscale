//! hnswsq2 insert — the Rust translation of pgvector's `hnswinsert.c`.
//!
...
pub unsafe fn load_neighbors(
    ...
    for tid in tids.iter().take(lm) {
        ...
        let mut hp = crate::access_method::hnswsq2::ptr::HnswPtr {
...
unsafe fn load_elements_for_insert(
    ...
    for i in 0..(*na).length as usize {
        ...
        let element =
            crate::access_method::hnswsq2::ptr::access::<Element>(std::ptr::null_mut(), hc.element);
...
pub unsafe fn insert_tuple_on_disk(
    ...
    // Create an element
    let level = entropy_level(get_ml(m), get_max_level(m));
    ...
    crate::access_method::hnswsq2::ptr::store(std::ptr::null_mut(), &mut (*element).value, value_ptr);
...
pub unsafe extern "C-unwind" fn aminsert(
    ...
    let mut insert_ctx = PgMemoryContexts::new("hnswsq2 insert temporary context");
