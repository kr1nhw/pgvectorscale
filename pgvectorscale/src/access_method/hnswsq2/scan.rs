//! hnswsq2 index scan — the Rust translation of pgvector's `hnswscan.c`
...
use crate::access_method::hnswsq2::options::HNSW2_EF_SEARCH;
use crate::access_method::hnswsq2::options::{
    HNSW2_ITERATIVE_SCAN, HNSW2_MAX_SCAN_TUPLES, HNSW2_SCAN_MEM_MULTIPLIER,
...
unsafe fn get_scan_items(state: &mut ScanState, index: pg_sys::Relation) -> Vec<SearchCandidate> {
    ...
    let entry_level = entry.level as usize;
    ...
    let ef = (HNSW2_EF_SEARCH.get() as usize).max(1);
...
unsafe fn resume_scan_items(state: &mut ScanState, index: pg_sys::Relation) -> Vec<SearchCandidate> {
    let load_vec = scan_load_vec(state.support.precision);
    let batch_size = (HNSW2_EF_SEARCH.get() as usize).max(1);
...
unsafe fn emit_distance(state: &ScanState, sc: &SearchCandidate) -> f64 {
    ...
    let element = crate::access_method::hnswsq2::ptr::access::<Element>(base, sc.element);
    let vec_bytes = state.support.codec.vector_bytes();
...
pub unsafe extern "C-unwind" fn ambeginscan(
    ...
    let m = get_m(index);
    let tmp_ctx = PgMemoryContexts::new("hnswsq2 scan temporary context");
    ...
    let max_memory = ((pg_sys::work_mem as f64) * HNSW2_SCAN_MEM_MULTIPLIER * 1024.0 + 256.0)
        as usize;
...
pub unsafe extern "C-unwind" fn amgettuple(
    ...
    if state.first {
        ...
        if (*scan).orderByData.is_null() {
            error!("cannot scan hnswsq2 index without order");
        }
        ...
        if (*(*scan).xs_snapshot).snapshot_type != pg_sys::SnapshotType::SNAPSHOT_MVCC {
            error!("non-MVCC snapshots are not supported with hnswsq2");
        }
        ...
        if HNSW2_ITERATIVE_SCAN.get().as_i32() != ITERATIVE_SCAN_OFF {
    ...
    let iterative = HNSW2_ITERATIVE_SCAN.get().as_i32();
    ...
    loop {
        ...
        if state.w.is_empty() {
            ...
            if state.tuples >= HNSW2_MAX_SCAN_TUPLES.get() as i64
                && HNSW2_MAX_SCAN_TUPLES.get() >= 0
        ...
        sc = *sc_ref;
        element = crate::access_method::hnswsq2::ptr::access::<Element>(base, sc.element);
