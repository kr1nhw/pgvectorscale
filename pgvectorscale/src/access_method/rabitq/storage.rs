...
pub struct RabitqStorage<'a> {
    ...
    qv_cache: Option<RefCell<RabitqCodeCache>>,
    has_labels: bool,
    num_neighbors: u32,
}

impl<'a> RabitqStorage<'a> {
    fn rabitq_vec_len(meta_page: &MetaPage) -> usize {
        (meta_page.get_num_dimensions_to_index() as usize).div_ceil(64)
    }
    ...
    pub unsafe fn new_for_build<S: StatsNodeRead>(
        ...
        let quantizer = RabitqMetadata::load(index, meta_page, stats);
        Self {
            index,
            ...
            qv_cache: Some(RefCell::new(RabitqCodeCache::new(
    ...
    fn load_quantizer<S: StatsNodeRead>(
        ...
        unsafe { RabitqMetadata::load(index_relation, meta_page, stats) }
    }
    ...
    fn visit_lsn_internal(
        ...
        lsr: &mut ListSearchResult<
            <RabitqStorage<'a> as Storage>::QueryDistanceMeasure,
            <RabitqStorage<'a> as Storage>::LSNPrivateData,
        >,
        ...
        match gns {
            GraphNeighborStore::Disk => {
                ...
                for &neighbor_index_pointer in neighbors.iter() {
                    ...
                    let code = node_neighbor.get_rabitq_code();
                    let distance = lsr
                        .sdm
                        .as_ref()
                        .expect("sdm is Some")
                        .calculate_rabitq_distance(&code, gns, &mut lsr.stats);
            ...
            GraphNeighborStore::Builder(b) => {
                ...
                for neighbor in neighbors.iter() {
                    ...
                    let code = cache.get(neighbor_index_pointer, self, &mut lsr.stats);
                    let distance = lsr
                        .sdm
                        .as_ref()
...
impl Storage for RabitqStorage<'_> {
    ...
    where
        Self: 'b;
    type LSNPrivateData = RabitqStorageLsnPrivateData; //no data stored
    ...
    fn create_node<S: StatsNodeWrite>(
        ...
        let code = self.quantizer.quantize(full_vector);

        let node = RabitqNode::with_meta(heap_pointer, meta_page, &code, labels);
    ...
    fn get_query_distance_measure(&self, query: LabeledVector) -> RabitqSearchDistanceMeasure {
        RabitqSearchDistanceMeasure::new(&self.quantizer, query)
    }
...
pub type RabitqStorageLsnPrivateData = PhantomData<bool>; //no data stored
